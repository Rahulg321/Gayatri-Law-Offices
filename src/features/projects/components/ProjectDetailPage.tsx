    ...
    <main className="page-wrap px-4 pb-16 pt-28 sm:pt-32">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-2">
          <Badge
            variant="secondary"
            className="rounded-full bg-[var(--gold-pale)] text-[11px] text-[var(--gold-deep)]"
          >
            {project.category}
          </Badge>
          <Badge
            variant="outline"
            className="rounded-full border-[var(--line)] text-[11px] font-normal capitalize text-[var(--charcoal-soft)]"
          >
            {formatProjectStatusLabel(project.projectStatus)}
          </Badge>
          {project.featured ? (
            <Badge className="rounded-full bg-[var(--gold)] text-[11px] text-white">
              Featured
            </Badge>
          ) : null}
        </div>
        ...
        {dateLineParts.length ? (
          <p className="mb-2 text-sm text-[var(--slate-soft)]">
            {dateLineParts.join(" · ")}
          </p>
        ) : null}
        ...
        <dl className="mb-8 flex flex-wrap gap-x-6 gap-y-2 text-sm text-[var(--slate-soft)]">
              ...
              <dd>
                {project.clientUrl?.trim() ? (
                  <a
                    ...
                    className="inline-flex items-center gap-1 text-[var(--gold-deep)] no-underline hover:underline"
                  >
                    {project.clientName}
          ...
          <div>
            ...
            <dd className="capitalize">
              {formatProjectStatusLabel(project.projectType)}
            </dd>
          </div>
        </dl>
        ...
        {hero ? (
          <img
            src={hero}
            alt=""
            className="mb-8 aspect-[16/9] w-full rounded-xl border border-[var(--line)] object-cover"
          />
        ) : null}
          ...
          <div className="mb-10 flex flex-wrap gap-2">
            {project.tags.map((tag) => (
              <Badge
                key={tag}
                variant="outline"
                className="rounded-full border-[var(--line)] font-normal text-[var(--charcoal-soft)]"
              >
                {tag}
              </Badge>
            ))}
        ...
        {project.bodyMarkdown.trim() ? (
          <Markdown
            content={project.bodyMarkdown}
            className="prose prose-lg mb-14 max-w-none text-[var(--charcoal-soft)]"
          />
        ) : null}

        {project.gallery.length ? (
          <section className="mb-14">
            <h2 className="display-title mb-6 text-xl font-semibold text-[var(--charcoal)]">
              Gallery
            </h2>
            <div className="columns-1 gap-4 sm:columns-2">
              {project.gallery.map((img) => (
                ...
                >
                  ...
                  />
                  {(img.caption ?? "").trim() ? (
                    <figcaption className="mt-2 text-center text-xs text-[var(--slate-soft)]">
                      {img.caption}
                    </figcaption>
                  ) : null}
                </figure>
                  ...
                  <div key={`${v.url}-${i}`} className="space-y-3">
                    ...
                    <div className="overflow-hidden rounded-xl border border-[var(--line)]">
                      {v.kind === "file" ? (
                        <video
                          controls
                          className="aspect-video w-full bg-black sm:max-h-[480px]"
                          src={v.url}
                        />
                      ) : null}
                      {v.kind === "youtube"
                        ? (() => {
                            ...
                            return src ? (
                              ...
                              <p className="p-4 text-sm text-[var(--charcoal-soft)]">
                                Could not embed this YouTube URL.
                              </p>
                            );
                          })()
                        : null}
                      {v.kind === "vimeo"
                        ? (() => {
                            ...
                            return src ? (
                              ...
                              <p className="p-4 text-sm text-[var(--charcoal-soft)]">
                                Could not embed this Vimeo URL.
                              </p>
                            );
                          })()
                        : null}
        ...
        {project.metrics.length ? (
          <section className="mb-14">
            <h2 className="display-title mb-4 text-xl font-semibold text-[var(--charcoal)]">
              Key results
            </h2>
        ...
        {project.challengesMarkdown.trim() ? (
          <section className="mt-14">
            ...
            <Markdown
              content={project.challengesMarkdown}
            ...
            >
              <Card className="feature-card h-full border-[var(--line)] transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] hover:-translate-y-1 hover:border-[var(--gold)]/30">
                <CardHeader className="pb-2">
                  ...
                  <CardTitle className="text-base font-semibold leading-snug text-[var(--charcoal)]">
                    {p.title}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="text-sm text-[var(--charcoal-soft)] line-clamp-3">
                    {p.excerpt}
                  </p>
                </CardContent>
              </Card>
